        ...
        resolveWebviewView(webviewView) {
            ...
            webviewView.webview.onDidReceiveMessage(async (message) => {
                ...
                if (message.type === 'fix-request') {
                    ...
                    try {
                        ...
                        if (relevantDiagnostics.length > 0) {
                            ...
                            replaceRanges = relevantDiagnostics.map(diag => {
                                ...
                                return new vscode.Range(new vscode.Position(startLine, 0), new vscode.Position(endLine, editor.document.lineAt(endLine).text.length));
                            });
                            // Remove duplicate/overlapping ranges
                            replaceRanges = replaceRanges.filter((range, idx, arr) => arr.findIndex(r => r.start.line === range.start.line && r.end.line === range.end.line) === idx);
                            // Sort by start line
                    ...
                    globalThis.vsai_lastReplaceRanges = replaceRanges;
                    globalThis.vsai_lastCodeBlocks = codeBlocks;
                    if (!code.trim()) {
                ...
                if (message.type === 'apply-fix') {
                    ...
                    let replaceRanges = globalThis.vsai_lastReplaceRanges || (lastReplaceRange ? [lastReplaceRange] : []);
